...
class WoocommerceUtil extends Util
{
    ...
    public function formatOrderToSale($business_id, $user_id, $order, $sell = null)
    {
        ...
        if (empty($order_customer_id)) {
            ...
            // Use billing information instead of customer account info for more accurate customer data
            $f_name = ! empty($order->billing->first_name) ? $order->billing->first_name : '';
            $l_name = ! empty($order->billing->last_name) ? $order->billing->last_name : '';
            $email = ! empty($order->billing->email) ? $order->billing->email : null;
            $phone = ! empty($order->billing->phone) ? $order->billing->phone : null;
            
            // If billing info is incomplete, fallback to customer account
            if (empty($f_name) && empty($l_name) && empty($email)) {
                ...
                $l_name = $order_customer->last_name;
        ...
        // First try to find customer by email if available
        ...
        // If no customer found by email, try to find by phone number
        if (empty($customer) && ! empty($customer_details['mobile'])) {
            $customer = Contact::where('business_id', $business_id)
        ...
        // If no customer found by email or phone, try to find by full name
        if (empty($customer) && ! empty(trim($customer_details['name']))) {
            $customer = Contact::where('business_id', $business_id)
                            ->where('name', trim($customer_details['name']))
                            ->OnlyCustomers()
                            ->first();
        }
        
        // Only use Walk-In Customer as absolute last resort when we have no useful customer info
        if (empty($customer) && empty(trim($customer_details['name'])) && empty($customer_details['email']) && empty($customer_details['mobile'])) {
        ...
        if (empty($customer)) {
            ...
            // Ensure we have a meaningful name for the customer
            $customer_name = trim($customer_details['name']);
            $first_name = $customer_details['first_name'];
            $last_name = $customer_details['last_name'];
            
            // If name is empty, try to construct from email or use mobile
            if (empty($customer_name)) {
                if (! empty($customer_details['email'])) {
                    $customer_name = $customer_details['email'];
                    $first_name = $customer_details['email'];
                } elseif (! empty($customer_details['mobile'])) {
                    $customer_name = 'Customer ' . $customer_details['mobile'];
                    $first_name = 'Customer';
                    $last_name = $customer_details['mobile'];
                } else {
    ...
    /**
     * Import categories from WooCommerce to POS
     */
    public function importWooCategories($business_id, $user_id)
    {
        try {
            $woocommerce = $this->woo_client($business_id);
            
            // Get all categories from WooCommerce
            $woo_categories = $this->getAllResponse($business_id, 'products/categories');
            ...
            foreach ($woo_categories as $woo_cat) {
                ...
                $existing_category = Category::where('business_id', $business_id)
                    ->where('name', $woo_cat->name)
                    ->first();
                
                if (!$existing_category) {
                    // Create new category
                    $category = new Category();
                    $category->name = $woo_cat->name;
                    $category->business_id = $business_id;
                    $category->created_by = $user_id;
                    $category->category_type = 'product';
                    $category->parent_id = 0; // Set as root category for now
                    $category->woocommerce_cat_id = $woo_cat->id;
                    $category->save();
                    
                    $imported_count++;
                } else {
                    // Update WooCommerce ID if not set
                    if (empty($existing_category->woocommerce_cat_id)) {
                        $existing_category->woocommerce_cat_id = $woo_cat->id;
            ...
            // Create sync log
            $this->createSyncLog($business_id, $user_id, 'import_categories', 'import', 
                ['imported' => $imported_count, 'skipped' => $skipped_count]);
            
            return [
                'success' => true,
                'message' => "Categories imported successfully! Imported: {$imported_count}, Skipped: {$skipped_count}"
            ];
    ...
    /**
     * Import products from WooCommerce to POS
     */
    public function importWooProducts($business_id, $user_id)
    {
        try {
            $woocommerce = $this->woo_client($business_id);
            ...
            $business = Business::find($business_id);
            $woocommerce_settings = json_decode($business->woocommerce_api_settings, true);
            $location_id = $woocommerce_settings['location_id'] ?? 1;
            
            // Get all products from WooCommerce
            $woo_products = $this->getAllResponse($business_id, 'products');
            ...
            foreach ($woo_products as $woo_product) {
                ...
                if (!in_array($woo_product->type, ['simple', 'variable'])) {
                    $skipped_count++;
                    continue;
                }
                ...
                if (!$existing_product) {
                    ...
                    if (!empty($woo_product->categories)) {
                        $woo_category = $woo_product->categories[0];
                        ...
                        if (!$category) {
                            ...
                            $category = new Category();
                            $category->name = $woo_category->name;
                            $category->business_id = $business_id;
                            $category->created_by = $user_id;
                            $category->category_type = 'product';
                            $category->parent_id = 0;
                            $category->woocommerce_cat_id = $woo_category->id;
                            $category->save();
                        }
                        $category_id = $category->id;
                    }
                    
                    // Get default unit (Pieces)
                    $default_unit = \App\Unit::where('business_id', $business_id)
                        ->orWhereNull('business_id')
                        ->first();
                    
                    // Determine product type based on WooCommerce type
                    $product_type = $woo_product->type === 'variable' ? 'variable' : 'single';
                    ...
                    $product->unit_id = $default_unit ? $default_unit->id : 1;
                    $product->sku = !empty($woo_product->sku) ? $woo_product->sku : 'WOO-' . $woo_product->id;
                    $product->product_description = strip_tags($woo_product->description ?? '');
                    $product->woocommerce_product_id = $woo_product->id;
                    $product->save();
                    
                    if ($woo_product->type === 'variable') {
                        // Handle variable product - get variations from WooCommerce
                        try {
                            $woo_variations = $this->getAllResponse($business_id, "products/{$woo_product->id}/variations");
                            
                            if (!empty($woo_variations)) {
                                foreach ($woo_variations as $woo_variation) {
                                    // Create product variation
                                    $product_variation = new \App\ProductVariation();
                                    $product_variation->name = $product->name;
                                    $product_variation->product_id = $product->id;
                                    $product_variation->save();
                                    
                                    // Create variation with attributes name
                                    $variation_name = !empty($woo_variation->attributes) ? 
                                        implode(', ', array_map(function($attr) { 
                                            return $attr->option ?? 'N/A'; 
                                        }, $woo_variation->attributes)) : 'Default';
                                    
                                    $variation = new \App\Variation();
                                    $variation->name = $variation_name;
                                    $variation->product_id = $product->id;
                                    $variation->product_variation_id = $product_variation->id;
                                    $variation->sub_sku = !empty($woo_variation->sku) ? $woo_variation->sku : 'WOO-VAR-' . $woo_variation->id;
                                    $variation->woocommerce_variation_id = $woo_variation->id;
                                    $variation->default_sell_price = floatval($woo_variation->price ?? 0);
                                    $variation->sell_price_inc_tax = floatval($woo_variation->price ?? 0);
                                    $variation->save();
                                    
                                    // Create variation location details for stock
                                    $variation_location = new \App\VariationLocationDetails();
                                    $variation_location->product_id = $product->id;
                                    $variation_location->product_variation_id = $product_variation->id;
                                    $variation_location->variation_id = $variation->id;
                                    $variation_location->location_id = $location_id;
                                    $variation_location->qty_available = max(0, $woo_variation->stock_quantity ?? 0);
                                    $variation_location->save();
                                }
                            } else {
                                // No variations found, create a default one
                                $this->createDefaultVariation($product, $location_id, $woo_product);
                            }
                        } catch (\Exception $e) {
                            // If variations API fails, create a default variation
                            $this->createDefaultVariation($product, $location_id, $woo_product);
                        }
                    } else {
                    ...
                    // Create product location entry for ForLocation scope
            ...
            // Create sync log
            $this->createSyncLog($business_id, $user_id, 'import_products', 'import', 
                ['imported' => $imported_count, 'skipped' => $skipped_count]);
            
            return [
                'success' => true,
                'message' => "Products imported successfully! Imported: {$imported_count}, Skipped: {$skipped_count}"
            ];
     ...
     */
    private function createDefaultVariation($product, $location_id, $woo_product)
    {
        // Create product variation
        $product_variation = new \App\ProductVariation();
        $product_variation->name = $product->name;
        ...
        $variation->product_id = $product->id;
        $variation->product_variation_id = $product_variation->id;
        $variation->sub_sku = !empty($woo_product->sku) ? $woo_product->sku : 'WOO-' . $woo_product->id;
        $variation->woocommerce_variation_id = $woo_product->id;
        $variation->default_sell_price = floatval($woo_product->price ?? 0);
        $variation->sell_price_inc_tax = floatval($woo_product->price ?? 0);
        $variation->save();
        ...
        $variation_location->variation_id = $variation->id;
        $variation_location->location_id = $location_id;
        $variation_location->qty_available = max(0, $woo_product->stock_quantity ?? 0);
        $variation_location->save();
    }
    
    /**
     * Fix missing variations for existing variable products
     */
    public function fixMissingVariations($business_id, $user_id)
    {
        try {
            $woocommerce = $this->woo_client($business_id);
            ...
            // Get all variable products from POS that have WooCommerce IDs
            $variable_products = Product::where('business_id', $business_id)
                ->where('type', 'variable')
            ...
            foreach ($variable_products as $product) {
                ...
                $woo_variations = $this->getAllResponse($business_id, "products/{$product->woocommerce_product_id}/variations");
                
                // Get existing variation IDs in POS
                $existing_woo_var_ids = $product->variations()
                    ->whereNotNull('woocommerce_variation_id')
                    ->pluck('woocommerce_variation_id')
                    ->toArray();
                
                $added_for_this_product = 0;
                
                foreach ($woo_variations as $woo_variation) {
                    // Skip if variation already exists
                    if (in_array($woo_variation->id, $existing_woo_var_ids)) {
                        continue;
                    }
                    ...
                    $added_for_this_product++;
                    $total_variations_added++;
                }
                
                if ($added_for_this_product > 0) {
                    $fixed_count++;
                }
            }
            
            // Create sync log
            $this->createSyncLog($business_id, $user_id, 'fix_variations', 'import', 
                ['products_fixed' => $fixed_count, 'variations_added' => $total_variations_added]);
            
            return [
                'success' => true,
                'message' => "Fixed variations for {$fixed_count} products. Added {$total_variations_added} missing variations."
            ];
    ...
    /**
     * Fix customer information for existing WooCommerce orders
     */
    public function fixCustomerData($business_id)
    {
        $fixed_count = 0;
        $woocommerce = $this->woo_client($business_id);
        
        try {
            // Get all transactions that have WooCommerce order IDs
            ...
            foreach ($transactions as $transaction) {
                try {
                    ...
                    $order = $woocommerce->get('orders/' . $transaction->woocommerce_order_id);
                    
                    // Process customer with the new logic
                    $order_customer_id = $order->customer_id;
                    
                    if (empty($order_customer_id)) {
                        // Guest customer
                        $customer_details = [
                            'first_name' => !empty($order->billing->first_name) ? $order->billing->first_name : '',
                            'last_name' => !empty($order->billing->last_name) ? $order->billing->last_name : '',
                            'email' => !empty($order->billing->email) ? $order->billing->email : null,
                            'name' => trim($order->billing->first_name . ' ' . $order->billing->last_name),
                            'mobile' => !empty($order->billing->phone) ? $order->billing->phone : null,
                        ];
                    } else {
                        // Use billing information instead of customer account info
                        $f_name = !empty($order->billing->first_name) ? $order->billing->first_name : '';
                        $l_name = !empty($order->billing->last_name) ? $order->billing->last_name : '';
                        $email = !empty($order->billing->email) ? $order->billing->email : null;
                        $phone = !empty($order->billing->phone) ? $order->billing->phone : null;
                    ...
                    // Find or create customer with corrected information
                    // Use unique combination of name+mobile to identify customers instead of just email
                    $contact = null;
                    
                    // First try to find by name + mobile combination for more accurate matching
                    if (!empty($customer_details['name']) && !empty($customer_details['mobile'])) {
                        $contact = Contact::where('business_id', $business_id)
                                        ->where('name', $customer_details['name'])
                                        ->where('mobile', $customer_details['mobile'])
                                        ->where('type', 'customer')
                                        ->first();
                    }
                    
                    // If not found and mobile exists, try mobile only
                    if (empty($contact) && !empty($customer_details['mobile'])) {
                        $contact = Contact::where('business_id', $business_id)
                                        ->where('mobile', $customer_details['mobile'])
                                        ->where('type', 'customer')
                                        ->first();
                        
                        // If found but name is different, update the customer
                        if ($contact && $contact->name != $customer_details['name']) {
                            $contact->name = $customer_details['name'];
                            $contact->first_name = $customer_details['first_name'];
                            $contact->last_name = $customer_details['last_name'];
                            $contact->save();
                        }
                    }
                    ...
                    // Update transaction with correct customer
                    ...
                    \Log::error("Error fixing customer data for order {$transaction->woocommerce_order_id}: " . $e->getMessage());
                    continue;
                }
            }
            ...
            \Log::error('Error fixing customer data: ' . $e->getMessage());
            return false;
        }
    }

    /**
     * Import orders from WooCommerce (without sync date filtering)
     */
    public function importWooOrders($business_id)
    {
        try {
            $imported_count = 0;
            $woocommerce = $this->woo_client($business_id);
            ...
            $existing_orders = Transaction::where('business_id', $business_id)
                                        ->whereNotNull('woocommerce_order_id')
                                        ->pluck('woocommerce_order_id')
                                        ->toArray();
            
            // Get all orders from WooCommerce with pagination
            $orders = $this->getAllResponse($business_id, 'orders', ['per_page' => 100]);
            ...
            foreach ($orders as $order) {
                ...
                // Skip draft orders
                if (in_array($order->status, ['auto-draft'])) {
                    continue;
                }
                
                // Create new sale from order
                $created = $this->createNewSaleFromOrder($business_id, 1, $order, $business_data);
                
                if ($created === true) {
                    $imported_count++;
                } else {
                    \Log::error("Failed to import WooCommerce order {$order->id}: " . (is_string($created) ? $created : 'Unknown error'));
                }
            }
            ...
            \Log::error('Error importing WooCommerce orders: ' . $e->getMessage());
            return false;
        }
    }
}

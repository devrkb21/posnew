...
class WoocommerceController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($logs)
                ...
                ->editColumn('sync_type', function ($row) {
                    ...
                    return array_key_exists($row->sync_type, $array) ? $array[$row->sync_type] : $row->sync_type;
                })
    ...
    public function importCategories()
    {
        if (request()->ajax()) {
            try {
                ...
                if (! (auth()->user()->can('superadmin') || ($this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module') && auth()->user()->can('woocommerce.sync_categories')))) {
                    abort(403, 'Unauthorized action.');
                }

                $result = $this->woocommerceUtil->importWooCategories($business_id, $user_id);
                
                $output = ['success' => 1, 'msg' => $result['message']];
            } catch (\Exception $e) {
                \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
    ...
    public function importProducts()
    {
        if (request()->ajax()) {
            try {
                ...
                $result = $this->woocommerceUtil->importWooProducts($business_id, $user_id);
     ...
     */
    public function fixMissingVariations()
    {
        $business_id = request()->session()->get('business.id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        try {
            $user_id = request()->session()->get('user.id');
            $result = $this->woocommerceUtil->fixMissingVariations($business_id, $user_id);
    ...
    /**
     * Fix customer data for existing WooCommerce orders
     */
    public function fixCustomerData()
    {
        $business_id = request()->session()->get('business.id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'woocommerce_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        try {
            $fixed_count = $this->woocommerceUtil->fixCustomerData($business_id);
            ...
            if ($fixed_count === false) {
                ...
                $output = [
                    'success' => 1,
                    'msg' => "Successfully fixed customer data for {$fixed_count} orders."
                ];
            }
        } catch (\Exception $e) {
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
    ...
    public function importOrders()
    {
        ...
        try {
            $imported_count = $this->woocommerceUtil->importWooOrders($business_id);
            ...
            if ($imported_count === false) {
                ...
                $output = [
                    'success' => 1,
                    'msg' => "Successfully imported {$imported_count} orders from WooCommerce."
                ];
            }
        } catch (\Exception $e) {
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());

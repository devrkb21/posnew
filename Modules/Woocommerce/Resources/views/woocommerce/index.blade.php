                                        ...
                                        <div class="tw-inline-block tw-min-w-full tw-py-2 tw-align-middle sm:tw-px-5">
                                            ...
                                            <div class="col-sm-6">
                                                <button type="button" class="tw-dw-btn tw-dw-btn-success tw-text-white tw-dw-btn-sm tw-dw-btn-wide"
                                                    id="import_product_categories"> <i class="fa fa-download"></i>
                                                    Import from WooCommerce</button>
                                                <span class="last_import_cat"></span>
                                            </div>
                                            <div class="col-sm-12">
                                                <br>
                            ...
                            <div class="">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    <div style="display: inline-flex; width: 100%; margin-bottom: 10px;">
                                        <button type="button" class="tw-dw-btn tw-dw-btn-success tw-text-white tw-dw-btn-sm" id="import_products">
                                            <i class="fa fa-download"></i> Import Products from WooCommerce</button>
                                        &nbsp;@show_tooltip('Import products and categories from WooCommerce into POS')
                                    </div>
                                    <span class="last_import_products"></span>
                                    <div style="display: inline-flex; width: 100%; margin-bottom: 10px;">
                                        <button type="button" class="tw-dw-btn tw-dw-btn-warning tw-text-white tw-dw-btn-sm" id="fix_missing_variations">
                                            <i class="fa fa-wrench"></i> Fix Missing Variations</button>
                                        &nbsp;@show_tooltip('Fix missing product variations for variable products')
                                    </div>
                                    <span class="last_fix_variations"></span>
                                    <div style="display: inline-flex; width: 100%; margin-bottom: 10px;">
                                        <button type="button" class="tw-dw-btn tw-dw-btn-info tw-text-white tw-dw-btn-sm" id="fix_customer_data">
                                            <i class="fa fa-users"></i> Fix Customer Data</button>
                                        &nbsp;@show_tooltip('Fix customer information for imported orders')
                                    </div>
                                    <span class="last_fix_customers"></span>
                                </div>
                                <div class="col-sm-12">
                ...
                @if ($is_superadmin || auth()->user()->can('woocommerce.sync_orders'))
                    <div class="col-sm-12">
                        @component('components.widget', ['class' => 'box-primary', 'title' => __('woocommerce::lang.sync_orders') . ':'])
                            <div class="col-sm-6">
                                <div style="display: inline-flex; width: 100%; margin-bottom: 10px;">
                                    <button type="button" class="tw-dw-btn tw-dw-btn-purple tw-text-white tw-dw-btn-sm" id="import_orders">
                                        <i class="fa fa-download"></i> Import Orders from WooCommerce</button>
                                    &nbsp;@show_tooltip('Import existing orders from WooCommerce into POS')
                                </div>
                                <span class="last_import_orders"></span>
                            </div>
                            <div class="col-sm-6">
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            //Import Categories
            $('#import_product_categories').click(function() {
                $(window).bind('beforeunload', function() {
                    return true;
                });
                var btn_html = $(this).html();
                $(this).html('<i class="fa fa-spinner fa-spin"></i> Importing...');
                $(this).attr('disabled', true);

                $.ajax({
                    method: "get",
                    url: "{{ action([\Modules\Woocommerce\Http\Controllers\WoocommerceController::class, 'importCategories']) }}",
                    dataType: "json",
                    timeout: 0,
                    success: function(result) {
                        if (result.success) {
                        ...
                        $('#import_product_categories').html(btn_html);
                        $('#import_product_categories').removeAttr('disabled');
                        $(window).unbind('beforeunload');
                    },
            ...
            $('#import_products').click(function() {
                ...
                $.ajax({
                    method: "get",
                    url: "{{ action([\Modules\Woocommerce\Http\Controllers\WoocommerceController::class, 'importProducts']) }}",
                    dataType: "json",
                    timeout: 0,
                    success: function(result) {
                        if (result.success) {
            ...
            $('#fix_missing_variations').click(function() {
                $(window).bind('beforeunload', function() {
                    return true;
                });
                var btn_html = $(this).html();
                $(this).html('<i class="fa fa-spinner fa-spin"></i> Fixing...');
                $(this).attr('disabled', true);

                $.ajax({
                    method: "get",
                    url: "{{ url('/woocommerce/fix-missing-variations') }}",
                    dataType: "json",
                    timeout: 0,
                    success: function(result) {
                        if (result.success) {
                        ...
                        $('#fix_missing_variations').html(btn_html);
                        $('#fix_missing_variations').removeAttr('disabled');
                        $(window).unbind('beforeunload');
                    },
                    error: function(xhr, status, error) {
                        toastr.error('Error: ' + error);
            ...
            // Fix Customer Data
            $('#fix_customer_data').click(function() {
                $(window).bind('beforeunload', function() {
                    return true;
                });
                var btn = $(this);
                ...
                $.ajax({
                    url: '{{action([\Modules\Woocommerce\Http\Controllers\WoocommerceController::class, 'fixCustomerData'])}}',
                    type: 'GET',
                    success: function(response) {
                        if (response.success) {
                            toastr.success(response.msg);
                            $('.last_fix_customers').text('Last fixed: {{date("M d, Y H:i")}}');
                        } else {
            ...
            $('#import_orders').click(function() {
                ...
                var btn = $(this);
                var btn_html = btn.html();
                btn.html('<i class="fa fa-spinner fa-spin"></i> Importing Orders...');
                btn.attr('disabled', true);

                $.ajax({
                    url: '{{action([\Modules\Woocommerce\Http\Controllers\WoocommerceController::class, 'importOrders'])}}',
                    type: 'GET',
                    timeout: 0,
                    success: function(response) {
                        if (response.success) {
                            toastr.success(response.msg);
                            $('.last_import_orders').text('Last imported: {{date("M d, Y H:i")}}');
                        } else {
                            toastr.error(response.msg);
                        }
                        btn.html(btn_html);

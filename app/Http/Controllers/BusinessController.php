    ...
    public function testSmsConfiguration(Request $request)
    {
        try {
            ...
            if (! empty($sms_settings['test_number'])) {
                ...
                if($parameter_type == 'json' && $response){
                    $body = $response->getBody()->getContents();
                    $json_response = json_decode($body, true);
                    
                    // Log the actual response for debugging
                    \Log::info('SMS API Response: ', ['response' => $json_response, 'status_code' => $response->getStatusCode()]);
                    
                    return ['success' => true, 'msg' => 'SMS sent successfully. Check logs for API response.', 'data' => $json_response];
                } else if ($response) {
                    return ['success' => true, 'msg' => 'SMS request sent. Response: ' . $response->getBody()->getContents()];
                } else {
                    return ['success' => false, 'msg' => 'Failed to send SMS. Check configuration.'];
                }
            } else {
